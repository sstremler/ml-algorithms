---
title: "NeuralNetwork for classification"
...
NNClassifier <- R6Class("NNClassifier",
  public = list(
    ...
    plotAnimation = function(step){
      if(length(step) == 1){
        step <- seq(1, length(nn$wSteps), step)
      }
      ...
      rangeX = seq(x1Min, x1Max, by = (x1Max - x1Min) / 200)
      rangeY = seq(x2Min, x2Max, by = (x2Max - x2Min) / 200)
      contour <- matrix(0, ncol = 4)[-1,]
      
      for(i in step){
        contourTemp <- data.matrix(expand.grid(x = rangeX, y = rangeY))
        contourTemp <- cbind(contourTemp, self$predict(contourTemp[,1:2], self$wSteps[[i]]), rep(i,dim(contourTemp)[1]))
        contour <- rbind(contour, contourTemp)
      }
      
      dfX = data.frame(self$X, self$y)
      colnames(dfX) <- c("b", "x1", "x2", "y")
      contour <- data.frame(x = contour[,1], y = contour[,2], z = contour[,3], time = contour[,4])
    ...
    plotError = function(){
      errDf <- data.frame(ce = nn$errorSteps, epoch = 1:self$epoch)
      print(ggplot(errDf, aes(x = epoch, y = ce)) + geom_line())
    },
...
nn$plotAnimation(c(1:15,50,100,500,1001))
