...
```{r}
kPctrPoly <- KernelPerceptron$new()
alpha <- kPctrPoly$train(X[,1:2], X[,3], kernel = polynomialKernel, kernelParam = 2)
print(alpha)
```
...
print(paste0("Steps: ", sum(alpha)))

kPctrPoly$plotAnimation()

```

## Train with gaussian kernel
```{r}
kPctrGauss <- KernelPerceptron$new()
alpha <- kPctrGauss$train(X[,1:2], X[,3], kernel = gaussianKernel, kernelParam = 1)
print(alpha)
```
